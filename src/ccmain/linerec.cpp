...
void Tesseract::LSTMRecognizeWord(const BLOCK& block, ROW *row, WERD_RES *word,
  ...
  if (lstm_dump_softmax){
    // name output file
    char buff[128];
    std::sprintf(buff, "./Softmax/Out-Softmax-%d-%d-%d-%d.bin", word_box.left(), ImageHeight() - word_box.top(),
                 word_box.right(), ImageHeight() - word_box.bottom());

    // create filehandle
    fout.open(buff, std::ios::out | std::ios::binary | std::ios::trunc);
  }
  ...
  lstm_recognizer_->RecognizeLine(*im_data, do_invert, classify_debug_level > 0,
                                  ...
                                  word_box, words, fout, lstm_choice_mode,
                                  lstm_dump_softmax);
  if (lstm_dump_softmax){
    fout.close();
  }
